...
public class NativePDFPrint {
	...
	static void print(final PDFFile pdfFile, PrintService service, final OrientationRequested orientation, Sides sides, int numCopies, float margins, float width, float height) {
		Printable pages = new Printable() {
			...
			public int print(Graphics g, PageFormat format, int index) throws PrinterException {
				...
				if (format.getImageableWidth() >= page.getWidth() || format.getImageableHeight() >= page.getHeight()) {
					bounds = new Rectangle((int) format.getImageableX(), (int) format.getImageableY(),
							(int) (page.getWidth()*change), (int) page.getHeight());
					g2.translate(0, 0);
				}
				else{
					bounds = new Rectangle((int) format.getImageableX(), (int) format.getImageableY(),
							(int) (format.getImageableWidth()*change), (int) format.getImageableHeight());
					g2.translate(0, 0);
				}
		...
		try {
			printJob.print(new SimpleDoc(pages, flavor, das), printRequestAttributeSet(orientation, sides, numCopies, width, height));
		} catch (PrintException e) {
			System.out.println("Error Printing: "+e.getMessage());
		}
	}

	private static PrintRequestAttributeSet printRequestAttributeSet(OrientationRequested orientation, Sides sides, int numCopies, float width, float height) {
